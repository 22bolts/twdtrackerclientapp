...
export default function UsersTable({
  className,
  usersData = [], // Change 'data' to 'usersData' for clarity
}: {
  className?: string;
  usersData: any[]; // Change 'data' to 'usersData' for clarity
}) {
  ...
  const {
    ...
    handleSelectAll,
    handleDelete,
  } = useTable(usersData, pageSize); // Pass 'usersData' instead of 'data'

  const columns = useMemo(
    () =>
      getColumns({
        usersData, // Pass 'usersData' instead of 'data'
        sortConfig,
  ...
  const selectedData = usersData.filter((item) => selectedRowKeys.includes(item.id));
